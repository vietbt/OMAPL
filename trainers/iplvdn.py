...
from algos.iplvdn import PolicyDiscrete, PolicyContinuous
...
class TrainerDiscrete(BaseTrainer):
    ...
    def __init__(self, model: PolicyDiscrete, logdir, dataset, n_agents, args):
        ...
        self.q_param = list(model.q.parameters())
    ...
    def update(self, data):
        ...
        pred_rewards = self.get_rewards(obs, next_obs, actions, dones)
        ...
        z = 1 / self._alpha * (q_target_values - v_values)
        ...
        v_loss = torch.exp(z - max_z) + torch.exp(-max_z) * v_values / self._alpha
        v_loss = v_loss.mean()
...
class TrainerContinuous(BaseTrainer):
    ...
    def update(self, data):
        ...
        pred_rewards = self.get_rewards(o_with_a_id, all_obs_with_id, dones)
        pred_rewards_x, pred_rewards_y = pred_rewards.chunk(2, dim=0)
        logits_x = pred_rewards_x.sum(1)
        logits_y = pred_rewards_y.sum(1)
        logits = logits_x - logits_y
